...
while (have_posts()):
  ...
  if ($show_title)
  {
    ...
    $title_content .= '<div class="post-title-wrapper"><h2 class="post-title">' . get_the_title() . '</h2></div>';
  ...
  if ($layout === 'sidebar_right' || $layout === 'sidebar_left')
  {
    ...
    else
    {
      if ($the_content !== '' && !empty($the_content))
      {
        ...
        if ($media_content !== '') $the_content =  $the_content . uncode_get_row_template($media_content, '', $limit_content_width, $style, '', false, false, false);
      }
      else
      {
    ...
    $main_content = '<div class="col-lg-' . $main_size . '">
                    <div class="post-content">
                      ' . $call_out . '
                      <h2>' . get_field('call_out_sentence') . '</h2>
                    </div>
                  ' . $the_content . '
                  <div class="listing-gallery">' . get_field('images') . '</div>
                </div>';
  ...
  if ($navigation_option !== 'off')
  {
    ...
    if ($navigation_index !== '')
    {
      ...
      $navigation_index_btn = '<a class="btn btn-link text-default-color" href="/apartments-for-sale/">' . ($navigation_index_label === '' ? get_the_title($navigation_index) : esc_html($navigation_index_label)) . '</a>';
    }
